...
# 绘制会员各级别人数条形图
fig = plt.figure(figsize = (8 ,5))  # 设置画布大小
plt.bar(x=range(3), height=[lv_four,lv_five,lv_six], width=0.4, alpha=0.8, color='skyblue')  # alpha 透明度，值越小越透明
...
plt.boxplot(age, 
            ...
            labels = ['会员年龄'],  # 设置x轴标题
            boxprops = {'facecolor':'lightblue'})  # 设置填充背景颜色
plt.title('会员年龄分布箱线图')
# 显示y坐标轴的底线
plt.grid(axis='y')
plt.show()
...
# 乘机信息类别
lte = data['LAST_TO_END'] # 最后一次乘机时间至观测窗口结束时长
fc = data['FLIGHT_COUNT'] # 观测窗口的飞行次数
sks = data['SEG_KM_SUM'] # 观测窗口的总飞行公里数
